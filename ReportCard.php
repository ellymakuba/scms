...
if(isset($_POST['period_id']) && isset($_POST['student_id']) && isset($_POST['PrintPDF']))
{
	...
	{	
		...
		$sql = "SELECT DISTINCT(rs.student_id),dtr.name,dtr.debtorno,gl.grade_level,
		dtr.grade_level_id,dtr.age,gl.id,dtr.balance 
		...
		$pdf->SetFont('times', '', 12, '', 'false');
		$LeftOvers = $pdf->addTextWrap(230,$YPos-($line_height*17),300,$FontSize, _('AVERAGE CLASS AGE').': ' .		
		$average_standard_age);
		...
		$pdf->SetFont('times', 'B', 12, '', 'false');
		$LeftOvers = $pdf->addTextWrap(500,$YPos-($line_height*16),300,$FontSize,$studentClass);
		$pdf->SetFont('times', '', 12, '', 'false');	
		$standard=$myrow[3];
		$YPos +=20;
		$YPos -=$line_height;
		...
		foreach ($bus_report->scheduled_subjects as $a => $b)
		 {
			...
			$cat_marks =$bus_report->average_cat_marks($b['subject_id'],$_POST['student_id'],$_POST['period_id'],$b['id'],$db);
...
{ /*The option to print PDF was not hit */
	...
	echo '<p class="page_title_text">' . ' ' . _('End Term Report Card') . '';
	if(isset($_GET['ID']))
	{
