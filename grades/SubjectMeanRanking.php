...
function get_subjects($class,$period,$db) {
	...
	$lower_display=$row[0];	
	if($lower_display==1){
		$sql = "select DISTINCT(subm.subject_id),subm.subject_id from subject_mean subm
...
class bus_report2 {
	...
	$lower_display=$row[0];	
	if($lower_display==1){
		$sql = "SELECT SUM(mean) AS class_mean, subm.subject_id as subid,subm.subject_id,sub.subject_name
