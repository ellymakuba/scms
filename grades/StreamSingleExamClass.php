...
function tep_get_subjects_marks_stream($subject_id,$student_id,$class,$period,$calendar_id,$exam_mode,$db) {
	...
	function set_calendar_vars_stream($class,$student_id,$period,$calendar_id,$exam_mode,$db) {
		...
		foreach ($subjects_array as $r=>$s) {
			$this->subject[] = array("id" => $s['id'],
								   ...
								   "tmarks" => tep_get_subjects_marks_stream($s['id'],$student_id,$class,$period, $this->calendar_id,$exam_mode,$db));
		}
	}
...
class bus_report_stream {
	...
	function get_scheduled_students_stream($class,$period,$exam_mode,$db) 
	{
		...
		$sql = "select rs.id,dm.name, rs.student_id,er.rank,dm.gender as initial,er.marks,er.no_of_subjects from registered_students rs
