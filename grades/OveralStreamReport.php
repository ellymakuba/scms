...
class bus_report_stream {
	...
	function get_scheduled_students_stream($class,$period,$db) {
		...
		$sql = "select rs.id,dm.name, rs.student_id,tsr.rank,dm.gender as
		initial,tsr.marks,tsr.no_of_subjects,tsr.mean,tsr.meanScore
			...
			while ($row = DB_fetch_array($result))
			 {
				...
				'initial' =>$row['initial'],'rank' =>$row['rank'],'marks' =>$row['marks'],'mean' =>$row['mean'],
