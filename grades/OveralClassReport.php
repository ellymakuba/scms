...
function tep_get_subjects($class,$period,$db) {
	...
	$result = DB_query($sql,$db);
	while ($row = DB_fetch_array($result)) {
		$subjects_array[] = array("id" => $row['id'],"subject_code" => $row['subject_code'],"subject_name" => $row['subject_name'],
		"department_id" => $row['department_id'],"grading" => $row['grading']);
	}
...
function set_calendar_vars($class,$student_id,$period,$calendar_id,$db) {
 ...
 class scheduled extends student {
	...
	function set_calendar_vars($class,$student_id,$period,$calendar_id,$db) {
		...
		$this->calendar_id = $calendar_id;
		// set status var
		$subjects = tep_get_subjects($class,$period,$db);
		foreach ($subjects as $r=>$s)
		 {
...
class bus_report {
	...
	function get_scheduled_students($class,$period,$db) {
		...
		$sql = "select rs.id,dm.name, rs.student_id,tcr.class_rank,tcr.marks,tcr.mean,dm.gender as initial,
		tcr.no_of_subjects,tcr.meanScore from registered_students rs,termly_class_ranks tcr,studentsmarks sm,debtorsmaster dm
		WHERE tcr.student_id=rs.student_id
		AND sm.calendar_id=rs.id
		AND dm.id=rs.student_id
		AND rs.period_id='$period'
		AND tcr.period_id='$period'
		AND rs.class_id IN ($streamsInClass)
		...
		if (DB_num_rows($result) > 0) {
			while ($row = DB_fetch_array($result))
			{
				$scheduled_students_array[] = array('id' => $row['id'],'student_id' => $row['student_id'],
				'totalmarks' => $row['totalmarks'],'name' => $row['name'],'marks' => $row['marks'],'mean' => $row['mean'],
				'initial' => $row['initial'],'class_rank' =>$row['class_rank'],'class_rank' =>$row['class_rank'],
				'no_of_subjects' =>$row['no_of_subjects'],'meanScore' =>$row['meanScore']);
			}
